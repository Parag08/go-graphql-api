...
import (
	...
	"github.com/parag08/go-graphql-api/gql"
	"github.com/parag08/go-graphql-api/postgres"
	"github.com/parag08/go-graphql-api/server"
)
...
func initializeAPI() (*chi.Mux, *postgres.Db) {
	...
	db, err := postgres.New(
		postgres.ConnString("localhost", 5432, "paragrahangdale", "go_graphql_db"),
	)
	if err != nil {
		log.Fatal(err)
	}
